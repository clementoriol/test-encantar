...
export class Utils {
  ...
  static assert(expr: boolean, errorMessage: string = ""): void {
    if (!expr) throw new AssertionError(errorMessage);
  }
  ...
  static range(n: number): number[] {
    if ((n |= 0) < 0) throw new IllegalArgumentError();
  ...
  static isIOS(): boolean {
    ...
    if (/(iOS|iPhone|iPad|iPod)/i.test(navigator.platform)) return true;
  ...
  static isWebKit(): boolean {
    ...
    if (/Apple/.test(navigator.vendor)) return true;
    ...
    if (/AppleWebKit\/.* Version\//.test(navigator.userAgent)) return true;

    // Chrome, Firefox, Edge on iOS
    if (/(CriOS\/|FxiOS\/|EdgiOS\/)/.test(navigator.userAgent)) return true;
