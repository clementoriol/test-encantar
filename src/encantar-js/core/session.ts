...
type SessionRequestAnimationFrameCallback = (
...
export class Session extends AREventTarget<SessionEventType> {
  ...
  private _rafQueue: Array<
    [SessionRequestAnimationFrameHandle, SessionRequestAnimationFrameCallback]
  >;
  ...
  static isSupported(): boolean {
    ...
    if (/(Mac|iOS|iPhone|iPad|iPod)/i.test(Utils.platformString())) {
      ...
      const ios =
        /(iPhone|iPad|iPod).* (CPU[\s\w]* OS|CPU iPhone|iOS) ([\d\._]+)/.exec(
          navigator.userAgent
        ); // Chrome, Firefox, Edge, Safari on iOS
      ...
      if (matches !== null) {
        ...
        if (x < 15 || (x == 15 && y < 2)) {
          Utils.error(
            `${
              matches === safari ? "Safari" : "iOS"
            } version ${version} is not supported! User agent: ${
              navigator.userAgent
            }`
          );
          return false;
  ...
  static instantiate(
    ...
    return Speedy.Promise.resolve()
        ...
        if (!isStableBuild) {
          if (
            ...
            if (
              ...
              const message =
                "This is a development build (unstable). Do not use it in production. Get a stable release at encantar.dev";
      ...
      .then(() => {
        ...
        return Speedy.Promise.all(sources.map((source) => source._init()));
      })
      .then(() => {
        // get the viewport
        if (!viewport)
   ...
   */
    ...
    if (!this._active) return Speedy.Promise.resolve();
  ...
  requestAnimationFrame(
    callback: SessionRequestAnimationFrameCallback
  ): SessionRequestAnimationFrameHandle {
  ...
  private _findPrimarySource(
    sources: Source[]
  ): Nullable<VideoSource | CanvasSource> {
    // prefer video sources
    for (let i = 0; i < sources.length; i++) {
      if (sources[i]._type == "video") return sources[i] as VideoSource;
    }
    for (let i = 0; i < sources.length; i++) {
      if (sources[i]._type == "canvas") return sources[i] as CanvasSource;
    }
  ...
  private _renderBackground(): void {
    ...
    for (let i = 0; i < this._trackers.length; i++) {
      ...
      if (media !== undefined) this._renderMedia(ctx, media, false);
    }
  ...
  private _renderMedia(
    ...
    // @ts-ignore
    if (media.type != "data") {
      const image = media.source as Exclude<
        SpeedyMediaSourceNativeElement,
        ImageData
      >;
