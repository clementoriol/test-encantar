...
export class ReferenceImageDatabase implements Iterable<ReferenceImage> {
  ...
  set capacity(value: number) {
    ...
    if (this.count > capacity)
      throw new IllegalArgumentError(
        `Can't set the capacity of the database to ${capacity}: it currently stores ${this.count} entries`
      );
  ...
  add(referenceImages: ReferenceImage[]): SpeedyPromise<void> {
    return this._preloadMany(referenceImages).then(
      (referenceImagesWithMedia) => {
  ...
  _addOne(referenceImage: ReferenceImageWithMedia): void {
    ...
    if (this._locked)
      throw new IllegalOperationError(
        `Can't add reference image "${name}" to the database: it's locked`
      );

    // reached full capacity?
    if (this.count >= this.capacity)
      throw new IllegalOperationError(
        `Can't add reference image "${name}" to the database: the capacity of ${this.capacity} images has been exceeded.`
      );
    ...
    if (
      ...
      throw new IllegalArgumentError(
        `Can't add reference image "${name}" to the database: invalid image`
      );

    // check for duplicate names
    if (this._entries.has(name))
      throw new IllegalArgumentError(
        `Can't add reference image "${name}" to the database: found duplicated name`
      );
  ...
  ): SpeedyPromise<ReferenceImageWithMedia> {
    if (referenceImage.name !== undefined)
      Utils.log(`Loading reference image \"${referenceImage.name}\"...`);
    else Utils.log(`Loading reference image...`);

    if (!referenceImage.image)
      return Speedy.Promise.reject(
        new IllegalArgumentError("The reference image was not provided!")
      );
  ...
  private _preloadMany(
    ...
    const promises = referenceImages.map((referenceImage) =>
      this._preloadOne(referenceImage)
    );
